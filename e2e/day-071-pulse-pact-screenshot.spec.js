const { test, expect } = require('@playwright/test');
const { spawn } = require('child_process');

const SERVER_DIR = '/home/peter/projects/daily-games/games/day-071-pulse-pact/server';
const WS_PORT = 13171;

test('capture Pulse Pact gameplay screenshot', async ({ browser }) => {
  const server = spawn('node', ['server.js'], {
    cwd: SERVER_DIR,
    env: { ...process.env, PORT: String(WS_PORT) },
    stdio: 'pipe',
  });

  await new Promise((resolve, reject) => {
    const timeout = setTimeout(() => reject(new Error('ws timeout')), 5000);
    server.stdout.on('data', (data) => {
      if (data.toString().includes('Pulse Pact server running')) {
        clearTimeout(timeout);
        resolve();
      }
    });
  });

  const hostCtx = await browser.newContext({ viewport: { width: 480, height: 640 } });
  const guestCtx = await browser.newContext({ viewport: { width: 480, height: 640 } });
  await hostCtx.addInitScript((port) => { window.__TEST_WS_URL = `ws://localhost:${port}`; }, WS_PORT);
  await guestCtx.addInitScript((port) => { window.__TEST_WS_URL = `ws://localhost:${port}`; }, WS_PORT);

  const host = await hostCtx.newPage();
  const guest = await guestCtx.newPage();

  await host.goto('/games/day-071-pulse-pact/');
  await host.fill('#player-name', 'Ada');
  await host.click('#btn-create');
  const roomCode = await host.locator('#lobby-code').textContent();
  expect(roomCode).toMatch(/^[A-Z]{4}$/);

  await guest.goto('/games/day-071-pulse-pact/');
  await guest.fill('#player-name', 'Ben');
  await guest.fill('#room-code-input', roomCode);
  await guest.click('#btn-join');
  await host.click('#btn-start');
  await host.waitForSelector('#screen-game.active');
  await host.waitForTimeout(2600);

  for (let i = 0; i < 10; i += 1) {
    await host.locator('.lane-btn:not([disabled])').first().click();
    await guest.locator('.lane-btn:not([disabled])').first().click();
    await host.waitForTimeout(180);
  }

  await expect(host.locator('#score-value')).not.toHaveText('0');
  await host.screenshot({ path: 'games/day-071-pulse-pact/screenshot.png' });

  await hostCtx.close();
  await guestCtx.close();
  server.kill();
});
